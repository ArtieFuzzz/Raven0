...
class GenderfluidCommand extends Command {
	constructor(...args) {
		super(...args, {
			usage: '[user:user]',
		});
	...
	async run(message, [user = message.author]) {
		...
		await message.channel.send({ files: [`https://api.ravy.lgbt/circle?image=${av}&type=genderfluid`] });
		wait.delete();
	}
}

module.exports = GenderfluidCommand;